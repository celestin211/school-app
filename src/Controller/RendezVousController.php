...
#[Route(path: '/calendar', methods: ['GET'])]
class RendezVousController extends AbstractController
{
    /**
     * @param ScheduleRepository $scheduleRepository
     * @param RendezVousRepository $appointmentRepository
     * @param UtilisateurRepository $utilisateurRepository
     * @return Response
     * @throws \Doctrine\ORM\NoResultException
     * @throws \Doctrine\ORM\NonUniqueResultException
     */
    #[Route('professeur/calendar', name: "calendar", methods: ["GET"])]
    #[IsGranted('ROLE_PROFESSEUR')]
    public function index(ScheduleRepository $scheduleRepository, RendezVousRepository $appointmentRepository, UtilisateurRepository $utilisateurRepository): Response
    {
        $now = new DateTime();

        $user = $this->getUser()->getUsername();

        $iduser = $utilisateurRepository->findUtilisateurs($user);
     ...
     * @param RendezVousRepositoryt $appointmentRepository
     * @param ScheduleRepository $scheduleRepository
     * @return JsonResponse
     */
    public function activer(Request $request, RendezVousRepositoryt $appointmentRepository,
     ...
     */
    public function deleteRdv(int $scheduleid, RendezVousRepositoryt $appointmentRepository, ScheduleRepository $scheduleRepository): Response
    {
     ...
     * @return Response
     */
    public function listPatient(UtilisateurRepository $utilisateurRepository): Response
    {
        return $this->render('patient/list.html.twig', [
            'listPatients' => $utilisateurRepository->findAll()
     ...
     * @return Response
     */
    public function listPatientupdate(Utilisateur $utilisateur, Request $request): Response
    {

        $form = $this->createForm(ModifierEleveType::class, $utilisateur);
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash(
                "alert alert-danger",
                "Le patient {$utilisateur->getPatientNom()} a bien été modifié !"
            );
     ...
     * @IsGranted("ROLE_PROFESSUER")
     * @param int $listPatientid
     * @param ScheduleRepository $scheduleRepository
     * @param UtilisateurRepository $utilisateurRepository
     * @return Response
     */
    public function listPatientdelete(int $listPatientid, UtilisateurRepository $utilisateurRepository): Response
    {
        $entityManager = $this->getDoctrine()->getManager();

        $patients = $utilisateurRepository->find($listPatientid);
